...
int microstepDelay=500;       //intervalo de tempo entre oz pulsos 
...
typedef struct Rdados{        //Estrutura dos dados recebidos pelos escravos
  int x=0;                 //Quantidade de pulsos que o módulo deve atingir 
};
...
void OnDataRecv(const uint8_t * mac, const uint8_t *incomingData, int len) {
  ...
  remoto();                             //executa o movimento remotamente
  digitalWrite(led,LOW);                //desliga o led onboard
...
void remoto(){
  ...
  while(rotValue != myData.x && peso<300){            //permanece na condição até igualar a meta

    peso = pesagem.get_units();        // Lê o peso da célula de carga em unidades definidas
    Serial.println(peso);                   //printa o peso
  ...
  ledcDetachPin(STEP);                    //quando a meta for atingida o pino é desacoplado do pwm
}
//----------------------------------------
void corrige(){                           //função que corrige a posição do cbao quando estiver parado 

  pwmFreq=380;                            //define uma frequencia para o acionamento do motor
  ledcDetachPin(STEP);                    //
  ledcWriteTone(pwmChannel, pwmFreq);     //Configura o pwm para  a nova frequencia
  ledcAttachPin(STEP, pwmChannel);        //

  while(rotValue != myData.x){            //verifica o erro e seta a direção de giro do motor
...
void setup() {
  ...
  pinMode(led,OUTPUT);                           //Devine os pinos de entrada e sáida
