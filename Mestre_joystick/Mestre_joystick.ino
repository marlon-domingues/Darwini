...
void calculo_comprimento(){
  for (int i = 0; i < 4; i++){
    mot.rotValue = round(sqrt(pow(coord_modulos[i][0] - coord_x,2)+pow(coord_modulos[i][1] - coord_y,2)));
...
void IRAM_ATTR env_pos(void* arg) {
  val_x = map(analogRead(pino_x),0,4095,100,-100);
  val_y = map(analogRead(pino_y),0,4095,100,-100);


  if(abs(val_x) > 20 || abs(val_y) > 20){
    int modulo = sqrt(pow(val_x,2)+pow(val_y,2));

    //coord_x += val_x*2/modulo;
    //coord_y += val_y*2/modulo;

    coord_x += val_x*5/abs(val_x);
    //coord_y += val_y*5/abs(val_y);
...
//------------------TASK COMUNICAÇÃO PYTHON----------------------

void printar(void *parameter){
  while(true){
...
void setup() {
  ...
  ativar_int_tempo();
  xTaskCreatePinnedToCore(printar, "Comunicação python", 10000, NULL, 1, NULL, 0); // Cria a tarefa na core 0
...
void loop() {
  if(!esp_timer_is_active(timerHandle)){
    ...
    if(Serial.available()){
      coord_x = Serial.parseInt();
      Serial.readStringUntil(',');
      coord_y = Serial.parseInt();
      Serial.readStringUntil('\n');
      calculo_comprimento();
    }
  }
